...
const hotelsFile = fs.createWriteStream(path.join('.', 'server', 'db-postgres', 'seedData', 'hotels.txt'));
(async() => {
  for (let i = 1; i <= 10000000; i++) {
    ...
    row += faker.lorem.word() + ' ';
    row += faker.internet.url() + '\n';

    if (faker.random.boolean() && !hotelsFile.write(row)) {
      await new Promise(resolve => hotelsFile.once('drain', resolve));
    }
  }
})();

const langFile = fs.createWriteStream(path.join('.', 'server', 'db-postgres', 'seedData', 'hotelLanguages.txt'));
(async() => {
  for (let i = 1; i <= 10000000; i++) {
    for (var j = 1; j <= 8; j++) {
...
const featureFile = fs.createWriteStream(path.join('.', 'server', 'db-postgres', 'seedData', 'hotelFeatures.txt'));
(async() => {
  for (let i = 1; i <= 10000000; i++) {
    for (var j = 1; j <= 19; j++) {
...
const amenFile = fs.createWriteStream(path.join('.', 'server', 'db-postgres', 'seedData', 'hotelAmenities.txt'));
(async() => {
  for (let i = 1; i <= 10000000; i++) {
    for (var j = 1; j <= 41; j++) {
...
const roomFile = fs.createWriteStream(path.join('.', 'server', 'db-postgres', 'seedData', 'hotelRoomTypes.txt'));
(async() => {
  for (let i = 1; i <= 10000000; i++) {
    for (var j = 1; j <= 4; j++) {
...
const imagesFile = fs.createWriteStream(path.join('.', 'server', 'db-postgres', 'seedData', 'hotelImages.txt'));
(async() => {
  for (let i = 1; i <= 10000000; i++) {
    for (var j = 0; j < 8; j++) {
      // Change to my S3 bucket later
      let url = `https://tripadcobaabout.s3.us-east-2.amazonaws.com/image${faker.random.number({min: 1, max: 50})}.jpg`;
